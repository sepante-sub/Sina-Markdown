...
This project will result in the production of:

- a relational spatially-enabled database integrating all known surface water, ground water, and water quality data for the middle Rio Grande basin study area. 
- Additionally, Visual Basic for Applications (VBA) code and Structured Query Language (SQL) code are products of the project. 
- All updateable datasets are acquired from the original data source (for example, EPA websites). Updatable data sources are acquired at specified intervals – quarterly, or as needed. 
...
## Figures representing the Source Data

### The geographic area represented in this study is illustrated below.     
![Figure 1.](img/rio-grande-basin.png)    
[Figure courtesy of @Bartolino]


### Initial data from this investigation indicates interesting patterns in island size, based on a sample size of `r length(islands)` islands.


```{r plot-islands, echo=FALSE}
hist(islands)
...
Materials generated under the project will be disseminated in accordance with University/Participating institutional and NSF policies [see, for example, @Briney_2015]. Depending on such policies, materials may be
...
1. A  data  policy  and stewardship plan will be established. In addition to archiving, each database table will be exported to a delimited text format to ensure accessibility of the data by other software programs. 
1. The data manager at the NMISC will be responsible for the management of long-term storage and archived data

[^1]: The FGDC standard was chosen due to required Federal government standards.
